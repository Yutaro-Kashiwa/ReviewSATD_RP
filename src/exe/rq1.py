...
from exe import ENV
from modules.plot import plot_revisions
...
def accept_rate_ss(project, a, b, c, d):
    ...
    path = Path(f"{ENV['output_dir']}/{project}")
    path.mkdir(parents=True, exist_ok=True)
    out_df = pandas.DataFrame([accepted_header, accepted_p])  # accepted_eff
    out_df.to_csv(f"{ENV['output_dir']}/{project}/rq1_p_value_accept_rate.csv", mode='w', header=False)
...
def revision_ss(project, a, b):
    ...
    out_df.to_csv(f"{ENV['output_dir']}/{project}/rq1_p_value_revisions.csv", mode='w', header=False)


def run(project, df):
    plot_revisions(df, f"{ENV['output_dir']}/{project}/")
    print("**RQ1********************")
    ...
    out_df = pandas.DataFrame(
        ...
        columns=header)
    out_df.to_csv(f"{ENV['output_dir']}/{project}/rq1__summary.csv")
